...
const AbilitySystemClassAlias = (() => {
  ...
  const params = PluginParamsParser.parse(
    PluginManager.parameters(AbilitySystemPluginName),
    typeDefine
  );
  ...
  const EnableUsableAllSkillsByMapSceneSwitchId =
    params.EnableUsableAllSkillsByMapSceneSwitchId;
  ...
  class AbilitySystemUtils {
    ...
    static isAutoEquipSkill() {
      if (EnableAutoEquipSkillSwitchId > 0)
        return $gameSwitches.value(EnableAutoEquipSkillSwitchId);
      return false;
    }
  }
  ...
  class Scene_Ability extends Scene_MenuBase {
    ...
    isReady() {
      ...
      if (BackgroundImage) {
        if (BackgroundImage.FileName) {
          const backgroundImage1 = ImageManager.loadBitmap(
            "img/",
        ...
        if (BackgroundImage.BackgroundImage2) {
          for (const img2 of BackgroundImage.BackgroundImage2) {
            if (img2.FileName) {
              const backgroundImage2 = ImageManager.loadBitmap(
                "img/",
                img2.FileName
              );
    ...
    getBackgroundImage2(actorId) {
      if (BackgroundImage.BackgroundImage2) {
        const img2 = BackgroundImage.BackgroundImage2.find(
          (img2) => img2.ActorId === actorId
        );
    ...
    updateBackgroundImage2() {
      ...
      const backgroundImage2 = this.getBackgroundImage2(
    ...
    createEquipAbilitiesWindow() {
      ...
      this._windowEquipAbilities.setHandler(
        "ok",
        this.onEquipAbilitiesOk.bind(this)
      );
      this._windowEquipAbilities.setHandler(
        "cancel",
        this.onEquipAbilitiesCancel.bind(this)
      );
      this._windowEquipAbilities.setHandler(
        "select",
        this.onEquipAbilitiesSelect.bind(this)
      );
      this._windowEquipAbilities.setHandler(
        "pagedown",
    ...
    createHasAbilitiesWindow() {
      ...
      this._windowHasAbilities.setHandler(
        "ok",
        this.onHasAbilitiesOk.bind(this)
      );
      this._windowHasAbilities.setHandler(
        "cancel",
        this.onHasAbilitiesCancel.bind(this)
      );
    ...
    hasAbilitiesWindowRect() {
      ...
      const h =
        this.mainAreaBottom() -
        equipAbilitiesWindowRect.y -
        equipAbilitiesWindowRect.height;
      return new Rectangle(x, y, w, h);
    }
    ...
    onHasAbilitiesOk() {
      const targetSkill = this.actor().hasAbilitySkill(
        this._windowHasAbilities.index()
      );
      const targetSkillId = targetSkill ? targetSkill.id : null;
      const changed = this.actor().changeEquipAbilitySkill(
        this._windowEquipAbilities.index(),
        targetSkillId
      );
      if (changed) {
  ...
  class Window_AbilitiesBase extends Window_Selectable {
    ...
    drawSkill(index) {
      ...
      this.drawItemName(
        ...
        rect.x,
        rect.y,
        rect.width - this.costWidth()
      );
      if (EnableCost)
        this.drawCost(
          index,
          AbilitySystemUtils.getSkillCost(this.itemAt(index).id)
        );
    }
  ...
  class Window_HasAbilities extends Window_AbilitiesBase {
    ...
    isItemEnabled(index) {
      ...
      return this._actor.canChangeEquipAbilitySkill(
        this._slotIndex,
        this.itemAt(index).id
      );
    }
  ...
  Game_Actor.prototype.addedSkills = function () {
    ...
    if (this._addedSkillsReturnAllFlag) {
      ...
      if (
        ...
        $gameSwitches.value(EnableUsableAllSkillsByMapSceneSwitchId)
      ) {
  ...
  Game_Actor.prototype.equipAbilitySkillIds = function () {
    ...
    for (let i = 0; i < skillIds.length; i++) {
      skillIds[i] =
        this._equipAbilitySkills[i] == null
          ? null
          : this._equipAbilitySkills[i];
    }
    return skillIds;
  };
  ...
  Game_Actor.prototype.removeAbilitySkill = function (skillId) {
    ...
    this._equipAbilitySkills = this._equipAbilitySkills.map((id) =>
      id === skillId ? null : id
    );
    this._hasAbilitySkills = this._hasAbilitySkills.filter(
      (id) => id !== skillId
    );
  };

  Game_Actor.prototype.totalCost = function (
    equipAbilitySkills = this._equipAbilitySkills
  ) {
    const costs = equipAbilitySkills.map((id) =>
      id ? AbilitySystemUtils.getSkillCost(id) : 0
    );
  ...
  Game_Actor.prototype.changeEquipAbilitySkill = function (
    index,
    targetSkillId
  ) {
  ...
  Game_Actor.prototype.canChangeEquipAbilitySkill = function (
    index,
    targetSkillId
  ) {
  ...
  Game_Actor.prototype.doChangeEquipAbilitySkill = function (
    index,
    targetSkillId
  ) {
  ...
  PluginManager.registerCommand(
    ...
    (args) => {
      ...
      const value =
        params.VariableId > 0
          ? $gameVariables.value(params.VariableId)
          : params.Value;
      const actor = $gameActors.actor(params.ActorId);
